...
export const updateComment = (id, value) => async (dispatch) => {
  return new Promise(() => {
    axios.put(`/comments/${id}`, value)
      .then(res => res.data)
      .then(data => {
        dispatch({type: UPDATE_COMMENTS, payload: data})
      })
      .catch(error => console.log('error from AddComment => ', error))
  })
}


export const getCommentByProductId = (productId,callback) => async (dispatch) => {
  return new Promise(() => {
    axios.get(`/comments/${productId}`)
      ...
      .catch(error => {
        dispatch({ type: NO_COMMENTS, payload: error })
